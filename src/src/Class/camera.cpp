#include "../../include/Class/Camera.h"
#include <cmath>
#include <iostream>
#ifndef M_PI
...
Camera::Camera(float xCam, float yCam, float zCam, float xTar, float yTar, float zTar) {
    ...
    updateAngles();
    updateVect();
    // par default la caméra est en third person view :
    vue = -1;
    nb_train = 0;
}
...
void Camera::Look() {
    gluLookAt(
        posCam.getX(), posCam.getY(), posCam.getZ(),
...
void Camera::Keybord(unsigned char key, int , int) {
    switch (key) {
        ...
        case 'n': // page up
            vue++;
            if (vue == nb_train) {
                // On renvoie la vue en troisième personne
                vue = -1;
            }
            break;
        case 'b': // page down
            vue--;
            if (vue == -2) {
                // On renvoie la vue au dernier train
                vue = nb_train - 1;
            }
            break;
        case 27:
...
void Camera::changerPosition(Vecteur _posCam, Vecteur _posTar) {
    posCam = _posCam;
    posTar = _posTar;
}
